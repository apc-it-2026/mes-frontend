...
namespace TierMeeting
{
    public partial class DetailKaizenForm : MaterialForm
    {
        ...
        public DetailKaizenForm(DateTime currentDate, string dept, int type)
        {
            ...
            var tempDate = DateTime.DaysInMonth(currentDate.Year, currentDate.Month);
            this.currentDate = new DateTime(currentDate.Year, currentDate.Month, tempDate);
            this.type = type;
            this.dept = dept;
            InitializeComponent();
        }
        ...
        private void GetKaizenData()
        {
            ...
            if (Convert.ToBoolean(JsonConvert.DeserializeObject<Dictionary<string, object>>(ret)["IsSuccess"]))
            {
                ...
                dtJson.DefaultView.Sort = "perpeople desc";
                
                DataTable dt_source = Pivot(dtJson.DefaultView.ToTable());
                
                gridData.DataSource = dt_source;
                gridData.Columns[0].HeaderText = "区域";
        ...
        private DataTable Pivot(DataTable dtJson)
        {
            ...
            //改成接收份数、采纳份数、人均采纳份数3个
            dt1.Rows[0][lblDepartmentText.Text] = "接收份数";
            dt1.Rows[1][lblDepartmentText.Text] = "采纳份数";
            dt1.Rows[2][lblDepartmentText.Text] = "人均采纳份数";
        ...
        {
            var tempDate = DateTime.DaysInMonth(currentDate.Year, cbxMonth.Text.ToInt());
            this.currentDate = new DateTime(currentDate.Year, cbxMonth.Text.ToInt(), tempDate);
        }
        private void ColorManage()
        {
            for (int i = 1; i < gridData.Columns.Count; i++)
            {
                ...
                if (gridData.Rows[1].Cells[i].Value.ToDouble() / (gridData.Rows[1].Cells[i].Value.ToDouble() / gridData.Rows[2].Cells[i].Value.ToDouble() * 3 / 12) >=1)
                {
                    gridData.Rows[2].Cells[i].Style.BackColor = Color.Green;
                }
                else if (gridData.Rows[1].Cells[i].Value.ToDouble() / (gridData.Rows[1].Cells[i].Value.ToDouble() / gridData.Rows[2].Cells[i].Value.ToDouble() * 3 / 12) >= 0.95)
                {
