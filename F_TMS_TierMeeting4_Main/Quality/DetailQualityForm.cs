...
namespace TierMeeting
{
    public partial class DetailQualityForm : MaterialForm
    {
            ...
            if (Convert.ToBoolean(JsonConvert.DeserializeObject<Dictionary<string, object>>(ret)["IsSuccess"]))
            {
                ...
                switch (type)
                {
                    case (int)Parameters.QueryDeptType.All: 
                        gridData.Columns["colSection"].Visible = false;
                ...
                for (int i = 0; i < dtJson.Rows.Count; i++)
                {
                    if (RFTType=="L")
                    {
                        if (string.IsNullOrEmpty(dtJson.Rows[i]["RFT"].ToString()))
                        {
                            gridData.Rows[i].DefaultCellStyle.BackColor = Color.White;
                        }
                        else if (dtJson.Rows[i]["RFT"].ToDouble() >=90)
                        {
                    ...
                    else if (RFTType == "S")
                    {
                        ...
                        else if (dtJson.Rows[i]["RFT"].ToDouble() >= 97)
                        {
                            gridData.Rows[i].DefaultCellStyle.BackColor = Color.Green;//颜色管理
                        }
                    ...
                    else if (RFTType == "C")
                    {
                        ...
                        else if (dtJson.Rows[i]["RFT"].ToDouble() >= 98)
                        {
                            gridData.Rows[i].DefaultCellStyle.BackColor = Color.Green;//颜色管理
                        }
        ...
        private void gridData_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (flag && type == (int)Parameters.QueryDeptType.All && e.ColumnIndex.Equals(gridData.Columns["colPlant"].Index))
            {
                DetailQualityForm frm = new DetailQualityForm(dtp.Value, gridData.CurrentCell.Value.ToString(), (int)Parameters.QueryDeptType.Plant, RFTType);
                frm.Show();
            } else if (flag && type == (int)Parameters.QueryDeptType.Plant && e.ColumnIndex.Equals(gridData.Columns["colSection"].Index))
            {
