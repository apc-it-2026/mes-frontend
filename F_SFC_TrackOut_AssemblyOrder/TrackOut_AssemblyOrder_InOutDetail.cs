...
namespace F_SFC_TrackOut_AssemblyOrder
{
    public partial class TrackOut_AssemblyOrder_InOutDetail : MaterialForm
    {
        ...
        private void GetInOutDetail(string vOrgId, string d_dept, string se_id, string po)
        {
            ...
            p.Add("vPo", po);
            p.Add("vSeId", se_id);
            string ret = SJeMES_Framework.WebAPI.WebAPIHelper.Post(Program.Client.APIURL, "KZ_SFCAPI_WorkOrder", "KZ_SFCAPI_WorkOrder.Controllers.GeneralOrderServer", "GetInOutDetail", Program.Client.UserToken, Newtonsoft.Json.JsonConvert.SerializeObject(p));
