...
using StickBottomInputByOrder.Bean;
...
{
    public partial class StickBottomInputByOrder : MaterialForm
    {
        Bitmap smile = null;
        Bitmap cry = null;
        ...
        public StickBottomInputByOrder()
        {
            InitializeComponent();
        ...
        /// <summary>
        /// Load warehouse according to work center
        /// </summary>
        ...
        private void TextQuerySeID_PreviewKeyDown(object sender, PreviewKeyDownEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                if (!string.IsNullOrWhiteSpace(txtQrCode) && !string.IsNullOrEmpty(txtQrCode))
                {
        ...
        private bool updateFinshQty(string WorkOrder, string SizeNo, int qty, string scan_ip, string qrcode_id, string datetime, string vArtNo, string vSeDay, string vPO)
        {
            ...
            string ret = SJeMES_Framework.WebAPI.WebAPIHelper.Post(Program.Client.APIURL, "KZ_SFCAPI_WorkOrder", "KZ_SFCAPI_WorkOrder.Controllers.ProductionInputOrderServer", "updateFinshQty", Program.Client.UserToken, Newtonsoft.Json.JsonConvert.SerializeObject(p));
        ...
        private void listSize_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (listSize.SelectedIndex > -1)
            {
                try
                {
                    ...
                    /////
                    ///
                     //The completed quantity of the work order size of the department
                    daySizeFinishQty = GeSizeFinishQty(workson);
                    //Total unfinished quantity of work order size
                    int totalUnFinishQty = GeTotalSizeFinishQty(workson);
                    //The unfinished quantity of the work order size of the department
        ...
        private void butConfirm_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Please select Order/PO and SIZE", "mistake！");
                return;
            }

            if ("".Equals(textEnterQty.Text.ToString()) || decimal.Parse(textEnterQty.Text.ToString()) == 0)
            {
                MessageBox.Show("Please enter a transfer amount greater than 0", "mistake！");
                return;
            }
            daySizeWorkQty = GetDaySizeWorkQty(listSizeSelectIndex);
            if (label12.BackColor == Color.CornflowerBlue)
            ...
            {
                MessageBox.Show("The quantity entered is greater than the transferable quantity", "mistake！");
                return;
            }
            ...
            DialogResult dr = MessageBox.Show("Confirm submission data？", "hint", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
